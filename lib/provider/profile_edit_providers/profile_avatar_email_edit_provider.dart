...
class ProfileAvatarEmailEditProvider extends ChangeNotifier {
  ...
  Future<void> setSelectedAvatar(String avatarUrl, BuildContext context) async {
    ...
    if (user != null && !user.isAnonymous) {
      ...
      try {
        ...
        await _firestore.collection('userByEmailAuth').doc(uid).set({
          'avatarPhotoURL': avatarUrl,
        }, SetOptions(merge: true)).then((value) async {
          _isAvatarUpdated = true;

          // Refetch user details after update to reflect changes
  ...
  final TextEditingController nicknameControllerByEmail = TextEditingController();
    ...
    if (user != null && !user.isAnonymous) {
      ...
      try {
        ...
        if (userDoc.exists) {
          ...
          _avatarPhotoURL = userDoc['avatarPhotoURL'] ?? 'https://example.com/default-avatar.png'; // Fallback URL
